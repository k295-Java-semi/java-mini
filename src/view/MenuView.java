...
public class MenuView {
		...
		try {
			...
			for (Room room : rooms) {
				...
				System.out.printf("| %-7d | %-7s | %-4s | %-5s | %-11s | %-7d | %-9s |\n", room.getRoomId(),
						room.getRoomNumber(), room.getType(), sizeIn, currencyFormat.format(room.getPrice()), // 천 단위 쉼표
																												// 추가
						room.getCapacity(), availability);
			}
		} catch (Exception e) {
		...
		try {
			...
			for (Room room : rooms) {
				...
				System.out.printf("| %-7s | %-5s | %-4s | %,9d | %-7d | %-7s |\n", room.getRoomNumber(), room.getType(),
						room.getSize(), (int) room.getPrice(), room.getCapacity(), status);
			}
			System.out.println("----------------------------------------------------------");
			...
			if ("1".equals(choice)) {
				...
				Room selectedRoom = rooms.stream().filter(r -> r.getRoomNumber().equals(roomNumber) && r.isAvailable())
		...
		try {
			...
			for (Attraction attraction : attractions) {
				System.out.printf("| %-11d | %-20s | %-50s | %-8s |\n", attraction.getAttractionId(),
						attraction.getAttractionName(), attraction.getAddress(), attraction.isEntrance() ? "가능" : "불가");
			}
			System.out.println("----------------------------------------------------------");
		} catch (Exception e) {
			printError(e.getMessage());
	...
	private static void viewBookingHistory() {
		try {
			...
			for (Booking booking : bookings) {
				...
				List<BookingDetail> details = bookingDetailController
						.getBookingDetailById(bookings.get(count).getBookingId());
				...
				for (BookingDetail detail : booking.getBookingDetail()) {
					...
					System.out.printf("| %-9s | %-7s | %-11s | %-12s | %-6s | %-6s | %-9s |\n",
							detail.getBookingDetailId(), rooms.get(0).getRoomNumber(), detail.getCheckInDate(),
							detail.getCheckOutDate(), detail.getGuestCount(), detail.getTotalPrice(),
							rooms.get(0).isAvailable() ? "예약 가능" : "예약 중");
					System.out.println("--------------------------------------------");
				}
				count++;
			}
			...
			if ("1".equals(choice)) {
				System.out.println("예약 상세 번호를 입력하세요.");
				int bookngDetailId = Integer.parseInt(sc.nextLine());

				viewBookingDetail(bookngDetailId);
			}
		} catch (Exception e) {
			printError(e.getMessage());
		}
	...
	private static void viewBookingDetail(int bookingDetailId) {
		try {
			// Fetch booking detail by ID
			BookingDetail bookingDetail = bookingDetailController.getBookingDetailById(bookingDetailId).stream()
					.findFirst().orElse(null);
			System.out.println("ddd = " + bookingDetail);
			if (bookingDetail == null) {
				printError("유효하지 않은 예약 상세 번호입니다.");
				return;
			}
			Booking booking = bookingController.getBookingByBookingDetailId(bookingDetailId);

			// Fetch the associated room information
			Room room = roomController.getRoomById(booking.getRoomId()).stream().findFirst().orElse(null);
			System.out.println("ddd = " + room);
			if (room == null) {
				printError("예약에 대한 방 정보를 찾을 수 없습니다.");
				return;
			}

			// Display booking detail information
			System.out.println("--------------------------------");
			System.out.println("예약 상세:");
			System.out.println("---------------------------------------");
			System.out.printf("| %-8s | %-15s |\n", "방 번호", room.getRoomNumber());
			System.out.printf("| %-8s | %-15s |\n", "체크인", bookingDetail.getCheckInDate());
			System.out.printf("| %-8s | %-15s |\n", "체크아웃", bookingDetail.getCheckOutDate());
			System.out.printf("| %-8s | %-15d |\n", "인원수", bookingDetail.getGuestCount());
			System.out.printf("| %-8s | %,15d원 |\n", "총 가격", bookingDetail.getTotalPrice());
			System.out.println("---------------------------------------");
			...
			case "1":
				...
				bookingDetail.setCheckInDate(Date.valueOf(sc.nextLine()));
				System.out.print("새 체크아웃 날짜(YYYY-MM-DD): ");
				bookingDetail.setCheckOutDate(Date.valueOf(sc.nextLine()));
				bookingDetailController.updateBookingDetail(bookingDetail);
				System.out.println("예약 상세가 수정되었습니다.");
				break;
			case "2":
